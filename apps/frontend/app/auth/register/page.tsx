...
import { Lock, Mail, User, ArrowLeft, Loader2, Eye, EyeOff } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof document !== "undefined") {
        document.cookie = "press_new_user=1; path=/; max-age=86400; samesite=lax";
      }
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans flex flex-col items-center justify-center p-6 relative overflow-hidden">
      <Link href="/" className="absolute top-6 left-6 z-50 flex items-center gap-2 text-[10px] font-mono text-slate-500 uppercase tracking-widest hover:text-sky-400 transition-colors group">
      ...
      <div className="w-full max-w-[400px] relative z-10">
        <div className="flex flex-col items-center mb-12">
          <img src="/icon_no_bg.png" alt="PRESS Logo" className="h-6 w-auto object-contain drop-shadow-[0_0_15px_rgba(56,189,248,0.3)]" />
          <div className="flex items-center gap-2 mt-4">
            <span className="text-[9px] font-mono text-slate-600 uppercase tracking-[0.4em]">Create Profile</span>
          </div>
        </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-2">
              ...
              <div className={`relative transition-all duration-300 ${isFocused === "fullName" ? "scale-[1.01]" : ""}`}>
                ...
                <input type="text" placeholder="John Doe" onFocus={() => setIsFocused("fullName")} onBlur={() => setIsFocused("")} value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full bg-slate-950/50 border border-slate-900 rounded-2xl py-4 pl-12 pr-4 text-sm font-mono text-white outline-none focus:border-sky-500/30 transition-all placeholder:text-slate-800" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className={`relative transition-all duration-300 ${isFocused === "email" ? "scale-[1.01]" : ""}`}>
                ...
                <input type="email" placeholder="name@email.com" onFocus={() => setIsFocused("email")} onBlur={() => setIsFocused("")} value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-slate-950/50 border border-slate-900 rounded-2xl py-4 pl-12 pr-4 text-sm font-mono text-white outline-none focus:border-sky-500/30 transition-all placeholder:text-slate-800" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className={`relative transition-all duration-300 ${isFocused === "password" ? "scale-[1.01]" : ""}`}>
                ...
                <input type={showPassword ? "text" : "password"} placeholder="••••••••••••" onFocus={() => setIsFocused("password")} onBlur={() => setIsFocused("")} value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full bg-slate-950/50 border border-slate-900 rounded-2xl py-4 pl-12 pr-12 text-sm font-mono text-white outline-none focus:border-sky-500/30 transition-all placeholder:text-slate-800" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className={`absolute inset-y-0 right-0 pr-4 flex items-center transition-colors hover:text-sky-400 ${isFocused === "password" ? "text-sky-400" : "text-slate-600"}`}>
            ...
            <div className="space-y-2">
              ...
              <div className={`relative transition-all duration-300 ${isFocused === "confirmPassword" ? "scale-[1.01]" : ""}`}>
                ...
                <input type={showConfirmPassword ? "text" : "password"} placeholder="••••••••••••" onFocus={() => setIsFocused("confirmPassword")} onBlur={() => setIsFocused("")} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="w-full bg-slate-950/50 border border-slate-900 rounded-2xl py-4 pl-12 pr-12 text-sm font-mono text-white outline-none focus:border-sky-500/30 transition-all placeholder:text-slate-800" />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className={`absolute inset-y-0 right-0 pr-4 flex items-center transition-colors hover:text-sky-400 ${isFocused === "confirmPassword" ? "text-sky-400" : "text-slate-600"}`}>
            ...
            {/* Submit Button */}
            <button type="submit" disabled={loading} className="w-full group relative py-4 bg-sky-500 text-slate-950 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-lg shadow-sky-500/20 active:scale-[0.98] transition-all overflow-hidden mt-4 disabled:opacity-70">
      ...
      <footer className="mt-12 text-[10px] font-mono text-slate-800 uppercase tracking-[0.4em]">PRESS.SYS // VER_1.0.4</footer>
