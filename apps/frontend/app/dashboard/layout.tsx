  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="font-mono text-[8px] tracking-wider text-slate-600 uppercase">
        {status === "connected" ? "" : status === "disconnected" ? "OFFLINE" : "SYNC"}
      </span>
    </div>
  );
}
    ...
    <div className="flex min-h-screen bg-[#020617]">
      ...
      <aside className="hidden md:flex md:w-[220px] lg:w-[260px] flex-col border-r border-slate-800/60 bg-slate-950/50">
        ...
        <nav className="flex flex-col gap-1 px-3 py-4 flex-1">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group relative flex items-center gap-3 rounded-xl px-3 py-2.5 font-mono text-xs transition-all duration-200 ${
                  active
              ...
              >
                {active && (
                  <motion.div
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-sky-400"
          ...
          <button
            ...
            className="mt-4 flex items-center gap-3 rounded-xl border border-dashed border-sky-400/30 px-3 py-2.5 font-mono text-xs font-medium text-sky-400/70 transition-all hover:border-sky-400/50 hover:bg-sky-400/5 hover:text-sky-400"
          >
            <Plus size={16} />
            New Session
          </button>
        ...
        <div className="border-t border-slate-800/40 px-4 py-3 flex flex-col gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-2 py-1.5 font-mono text-[10px] text-slate-600 transition-colors hover:text-red-400"
          >
            <LogOut size={12} />
            Logout
          </button>
          ...
          <div className="flex h-14 items-center justify-between px-4">
            ...
            <div className="flex items-center gap-2 relative">
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-xl border border-sky-400/20 bg-sky-400/10 transition-colors hover:bg-sky-400/20"
              >
                ...
                {showProfileDropdown && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 top-10 z-50 flex w-48 flex-col overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-xl"
                    >
                      <Link
                        href="/dashboard/profile"
                        onClick={() => setShowProfileDropdown(false)}
                        className="flex items-center gap-3 px-4 py-3 text-sm text-slate-300 transition-colors hover:bg-slate-800 hover:text-slate-100"
                      >
        ...
        <header className="hidden md:flex sticky top-0 z-40 h-14 items-center justify-between border-b border-slate-800/40 bg-slate-950/80 backdrop-blur-sm px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] tracking-wider text-slate-600 uppercase">
              {NAV_ITEMS.find((item) => isActive(pathname, item.href, item.exact))?.label || "Dashboard"}
            </span>
          </div>
          <DbStatusDot status={dbStatus} />
        </header>
      ...
      <nav className="glass-strong fixed bottom-0 left-0 right-0 z-40 border-t border-slate-800/60 md:hidden">
        <div className="flex items-center justify-around py-1.5 pb-[max(0.375rem,env(safe-area-inset-bottom))]">
          ...
          <button
            onClick={() => setShowNew(true)}
            className="group relative flex h-12 w-12 -translate-y-3 items-center justify-center rounded-full bg-sky-400 font-mono text-slate-950 shadow-[0_0_20px_-5px_rgba(56,189,248,0.6)] transition-all hover:bg-sky-300 hover:scale-105 active:scale-95"
          >
        ...
        {showNew && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-sm rounded-2xl border border-sky-400/20 bg-slate-900 p-5 shadow-2xl"
            ...
            >
              ...
              <form onSubmit={handleCreate} className="flex flex-col gap-6">
                <input
                  ...
                  placeholder="e.g., Chest & Triceps, Pull Day…"
                  autoFocus
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none transition-colors focus:border-sky-400"
                />
                <button
                  type="submit"
                  disabled={creating || !title.trim()}
                  className="w-full rounded-xl bg-sky-400 py-3 font-mono text-sm font-semibold text-slate-950 transition-opacity hover:opacity-90 disabled:opacity-50"
                >
