    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            href="/dashboard/session"
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-800 text-slate-500 transition-colors hover:border-slate-700 hover:text-slate-300"
          >
            <ArrowLeft size={16} />
          </Link>
          <div>
      ...
      {/* Add Exercise Button */}
      <Link
        href={`/dashboard/session/newlog?sessionId=${session.id}`}
        className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-sky-400/30 bg-sky-400/5 py-3 font-mono text-xs font-semibold text-sky-400 transition-all hover:border-sky-400/50 hover:bg-sky-400/10"
      >
        <Plus size={16} />
        Add Exercise
      </Link>
      ...
      <div className="mt-4 flex justify-center">
        <button
          ...
          disabled={deleting}
          className="flex items-center gap-1.5 rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-2 font-mono text-xs text-red-400 transition-colors hover:border-red-500/40 hover:bg-red-500/10 disabled:opacity-40"
        >
          <Trash2 size={12} />
    ...
    >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-800 bg-slate-900/60 py-2.5 pl-9 pr-4 text-sm text-slate-200 placeholder-slate-600 outline-none transition-colors focus:border-sky-400/50"
        />
      </div>
      ...
      {!loading && filteredSessions.length === 0 && (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-800 py-16 text-center">
          ...
          <p className="text-sm text-slate-500">
            {searchQuery ? "No sessions match your search." : "No sessions recorded."}
          </p>
        </div>
      )}
          ...
          <div className="flex flex-col gap-2">
            {monthSessions.map((session, i) => {
              ...
              if (prev && prev.total_volume > 0) {
                sessionDelta =
                  ((session.total_volume - prev.total_volume) / prev.total_volume) * 100;
              }
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-800 bg-slate-950/60 font-mono text-[10px] font-bold text-slate-600">
                        {String(sessions.length - globalIndex).padStart(2, "0")}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-slate-200">
                          {session.title}
