...
export function ProjectionCard({ projection, index }: { projection: ExerciseProjection; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }} className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 transition-all hover:border-slate-700">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="font-mono text-[9px] text-slate-600">
            <LocalTime value={projection.lastLogDate} options={{ month: "short", day: "numeric" }} />
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2 text-center">
          <p className="font-mono text-[8px] font-medium tracking-wider text-slate-600 uppercase">Last</p>
        ...
        <div className="flex-1 rounded-lg border border-sky-400/15 bg-sky-400/5 px-3 py-2 text-center glow-border">
          <p className="font-mono text-[8px] font-medium tracking-wider text-sky-400/60 uppercase">Target</p>
      ...
      {/* Reason */}
      <p className="mt-2 font-mono text-[9px] text-slate-600 leading-relaxed">{projection.reason}</p>
    </motion.div>
  );
}
